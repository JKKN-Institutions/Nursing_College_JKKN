// DEP-34 — the nursing enquiry form. One component, mounted on several pages.
//
// The Arts version of this form went live on 2026-08-16 and was submitted end-to-end from a real
// browser the same day: the action returned the CRM-accepted branch, not the 409 branch and not
// the error branch. This is the same shape, pointed at the nursing institution.
//
...
import { submitNursingEnquiry, type NursingEnquiryState } from "@/app/actions/nursing-enquiry"
import { NURSING_PROGRAMMES } from "@/lib/nursing-programmes"

const ADMISSION_PHONE = "+919345855001"
const ADMISSION_PHONE_DISPLAY = "+91 93458 55001"

type Props = {
  /** Which page this instance sits on, e.g. "bsc-nursing". Tags the lead in the CRM. */
  sourcePage: string
...
export default function NursingEnquiryForm({
  sourcePage,
  defaultProgrammeId,
  heading = "Get a Call Back",
}: Props) {
  const [state, formAction, isPending] = useActionState<NursingEnquiryState | null, FormData>(
    submitNursingEnquiry,
    null
  )
  ...
  return (
    <form action={formAction} className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label htmlFor="nur-name" className="block text-white text-sm font-medium mb-1.5">
        ...
        <div>
          <label htmlFor="nur-phone" className="block text-white text-sm font-medium mb-1.5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label htmlFor="nur-programme" className="block text-white text-sm font-medium mb-1.5">
        ...
        <div>
          <label htmlFor="nur-district" className="block text-white text-sm font-medium mb-1.5">
      ...
      <div>
        <label htmlFor="nur-question" className="block text-white text-sm font-medium mb-1.5">
