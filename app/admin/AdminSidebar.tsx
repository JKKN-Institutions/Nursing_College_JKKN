...
export default function AdminSidebar({
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      ...
      <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
        {(userRole === 'staff' ? topNavLinks.filter(({ href }) => href === '/admin/events' || href === '/admin/faculty') : topNavLinks).map(({ href, label, icon: Icon }) => {
  ...
  return (
    <>
      ...
      {showSignOutModal && (
        <>
          <div className="fixed inset-0 bg-black/50 z-50" onClick={() => setShowSignOutModal(false)} />
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
            <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm">
              <h2 className="text-lg font-bold text-[#002309] mb-2">Sign Out</h2>
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="flex-1 px-4 py-2.5 rounded-xl bg-red-500 hover:bg-red-600 text-white text-sm font-medium transition"
                >
                  Yes, Sign Out
                </button>
              </div>
