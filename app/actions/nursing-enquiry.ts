// DEP-34 — server action behind the nursing enquiry form.
//
...
// @/lib/nursing-programmes because a 'use server' module may not export plain values.

"use server"

import {
  NURSING_INSTITUTION_ID,
  VALID_NURSING_PROGRAMME_IDS,
} from "@/lib/nursing-programmes"

const CRM_SLUG = "jkkn-admission-2026"
const CRM_SUBMIT = `https://www.jkkn.ai/api/public/forms/${CRM_SLUG}/submit`

export type NursingEnquiryState = {
  success: boolean
  message?: string
  error?: string
...
function validate(name: string, phone: string, programme: string) {
  const fieldErrors: NursingEnquiryState["fieldErrors"] = {}
  ...
  if (!VALID_NURSING_PROGRAMME_IDS.has(programme))
    fieldErrors.programme = "Please choose a programme."
  return fieldErrors
}

export async function submitNursingEnquiry(
  _prevState: NursingEnquiryState | null,
  formData: FormData
): Promise<NursingEnquiryState> {
  ...
  const payload = {
    formData: {
      ...
      district,
      institution_program: {
        institution_id: NURSING_INSTITUTION_ID,
    ...
    utmSource: "nursing.sresakthimayeil.jkkn.ac.in",
    utmMedium: "site-form",
    utmCampaign: sourcePage ? `nursing-${sourcePage}` : "nursing-site",
    campaignLinkId: null,
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error("[nursing-enquiry] CRM rejected submit", res.status, body)
      return {
        success: false,
        error:
    ...
    console.error("[nursing-enquiry] network/timeout reaching CRM", err)
    return {
      success: false,
      error:
